...
def main(config_file=None, log_level=None, rounds=50, number_of_sheep=15, wait=False):
    ...
    if config_file:
        ...
        if sheep_init_pos_limit <= 0 or sheep_move_dist <= 0 or wolf_move_dist <= 0 or rounds <= 0 or \
                number_of_sheep <= 0:
            raise ValueError("All values must be positive numbers")
        logging.debug(
            f"Values loaded from configuration file: Sheep InitPosLimit = {sheep_init_pos_limit}, Sheep MoveDist = "
            f"{sheep_move_dist}, Wolf MoveDist = {wolf_move_dist}")
